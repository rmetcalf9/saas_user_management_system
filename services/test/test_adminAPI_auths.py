...
import pytz
from TestHelperSuperClass import httpOrigin, testHelperAPIClient, env, tenantWithNoAuthProviders, sampleInternalAuthProv001_CREATE, internalUSerSufix
...
class test_adminAPIAuths(parent_test_api):
  ...
  def test_deleteInternalAuthBadKey(self):
    ...
    result = self.testClient.post(
      self.adminAPIPrefix + '/' + masterTenantName + '/auths',
      headers={ "Origin": httpOrigin, jwtHeaderName: self.getNormalJWTToken()},
      data=json.dumps(newAuthDICT),
      content_type='application/json'
    )
    self.assertEqual(result.status_code, 201, msg="Create auth failed - " + result.get_data(as_text=True))
    ...
    #Check we can log in with created auth
    loginTestResult = self.canWeLoginWithInternalAuth(
      newAuthDICT["authProviderGUID"],
      newAuthDICT["credentialJSON"]["username"],
      newAuthDICT["credentialJSON"]["password"],
      masterTenantName
    )
    self.assertEqual(loginTestResult.status_code, 200, msg='Could not log in - ' + result.get_data(as_text=True))
