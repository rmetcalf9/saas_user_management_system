...
from TestHelperSuperClass import tenantWithNoAuthProviders, sampleInternalAuthProv001_CREATE, env, internalUSerSufix, sampleInternalAuthProv001_CREATE_WithAllowUserCreation, getHashedPasswordUsingSameMethodAsJavascriptFrontendShouldUse, httpOrigin
import json
...
class test_loginapi_register(parent_test_api):
  ...
  def test_unableToLoginAsDeletedUser(self):
    ...
    self.assertEqual(registerResult.status_code, 201, msg="User not created: " + registerResult.get_data(as_text=True))
    resultJSON = json.loads(registerResult.get_data(as_text=True))
    createdUserID = resultJSON["UserID"]
